...
% 导入数据
% load ('时间序列预测数据.csv',1,1);
result  = csvread('时间序列预测数据.csv',1); % data=csvread('时间序列预测数据.csv');%csvread只能读取纯数据，data=csvread('时间序列预测数据.csv',0,0)从第0行第0列开始读取数据
sps_num = length(result);  % 样本个数 

% 设置参数
sp = 50; % 步长（数据滑窗）
ts = 251; % 时间跨度（向后预测的个数=100时，需要满足 sp + ts = 301，发现 sp小、 ts大时效果好）
