...
% 建立网络
layers = [
    imageInputLayer([sp, 1, 1])     % 输入层 
    convolution2dLayer([3, 1], 16)  % 卷积核大小为3*1 特征图16张
    batchNormalizationLayer         % 批归一化层
    reluLayer                       % Relu激活层
    convolution2dLayer([3, 1], 32)  % 卷积核大小为3*1 特征图32张
    batchNormalizationLayer         % 批归一化层
    reluLayer                       % Relu激活层
    convolution2dLayer([3, 1], 32)  % 卷积核大小为3*1 特征图32张
    batchNormalizationLayer         % 批归一化层
...
options = trainingOptions('adam', ...      % ADAM 梯度下降算法
    ...
    'LearnRateDropPeriod', 300, ...        % 300次训练后 学习率为初始学习率*学习率下降因子
